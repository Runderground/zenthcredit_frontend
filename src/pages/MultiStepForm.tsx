...
export default function MultiStepForm() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!residencia || !renda || !identidade) {
      toast.error("Todas as fotos são obrigatórias!");
      setLoading(false);
      return;
    }
    ...
    async function RegisterUser() {
      try {
        const { data } = await axios.post(
          `${import.meta.env.VITE_API_URL}/cadastros/register`,
          formData,
        );
        ...
        toast.error(
          "Verifique o tipo de arquivo, tamanho, e se tudo está preenchido corretamente.",
        );
      }
    }
    RegisterUser();
  ...
  return (
    <div className="flex flex-col items-center justify-center m-4 mt-20">
      <div className="flex flex-col text-center mb-8">
        <h1 className="font-bold text-3xl text-slate-600 uppercase">
          Registro
        </h1>
          ...
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)}>
                ...
                {step == 3 && (
                  <div className="grid grid-cols-1 gap-4">
                    <span className="bg-yellow-200 text-yellow-700 p-2 border border-yellow-400 rounded-lg text-sm">
                      Tamanho máximo dos Arquivos: 5MB | Tipos permitidos:
                      [.JPEG, .JPG, .PNG, .PDF]
                    </span>
                    <div className="flex flex-col gap-2">
                      ...
                      <span className="flex flex-1 text-xs text-blue-500 bg-blue-200 p-2 rounded-lg border border-blue-400 mb-2">
                        Deve ser uma conta de luz, água, telefone, etc. que
                        esteja o endereço visível com seu nome ou de um
                        terceiro. ( O CEP deve ser preenchido exatamente como o
                        da foto )
                      </span>
                      <div>
                        {image1 && 
                          <div className="mb-4">
                            {!containsPdf(residencia?.name) ? (
                              <img
                                src={image1}
                                alt="Comprovante de residência"
                                className="w-[200px] mb-2 rounded-lg"
                              />
                            ) : (
                              <div className="w-[200px] h-[200px] flex items-center flex-col justify-center select-none bg-red-500 mb-4 rounded-lg">
                                <h2 className="text-3xl text-white">PDF</h2>
                                <span className="text-xs text-white">{residencia?.name}</span>
                              </div>)}
                          </div>
                        }

                        <a
                          className="border py-3 px-4 border-slate-200 rounded-lg text-sm font-medium my-2"
                          onClick={() => residenciaBtn.current?.click()}
                        >
                          Escolher arquivo
                        </a>
                      </div>
                      <input
                        type="file"
                        ref={residenciaBtn}
                        onChange={() => handleChangeFileResidencia()}
                        hidden
                      />
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      <span className="flex flex-1  text-xs text-blue-500 bg-blue-200 p-2 rounded-lg border border-blue-400 mb-2">
                        Pode ser um extrato bancário, contracheque, declaração
                        de imposto de renda, etc.
                      </span>
                      <div>
                        {image2 && 
                          <div className="mb-4">
                            {!containsPdf(renda?.name) ? (
                              <img
                                src={image2}
                                alt="Comprovante de residência"
                                className="w-[200px] mb-2 rounded-lg"
                              ...
                              <div className="w-[200px] h-[200px] flex items-center flex-col justify-center select-none bg-red-500 mb-4 rounded-lg">
                                <h2 className="text-3xl text-white">PDF</h2>
                                <span className="text-xs text-white">{renda?.name}</span>
                              </div>)}
                          </div>
                        }
                        <a
                          className="border py-3 px-4 border-slate-200 rounded-lg text-sm font-medium my-2"
                          onClick={() => rendaBtn.current?.click()}
                        >
                          Escolher arquivo
                        </a>
                      </div>
                      <input
                        type="file"
                        onChange={() => handleChangeFileRenda()}
                        ref={rendaBtn}
                        hidden
                      />
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      <span className="flex flex-1 text-xs text-blue-500 bg-blue-200 p-2 rounded-lg border border-blue-400 mb-2">
                        Deve ser uma única foto do seu RG ou CNH aberto. ( O
                        nome, data de nascimento e CPF devem estar preenchidos
                        exatamente como está no RG/CNH )
                      </span>
                      <div>
                        {image3 && 
                        <div className="mb-4">
                          {!containsPdf(identidade?.name) ? (
                            <img
                              src={image3}
                              alt="Comprovante de residência"
                              className="w-[200px] mb-2 rounded-lg"
                            ...
                            <div className="w-[200px] h-[200px] flex items-center flex-col justify-center select-none bg-red-500 mb-4 rounded-lg">
                              <h2 className="text-3xl text-white">PDF</h2>
                              <span className="text-xs text-white">{identidade?.name}</span>
                            </div>)}
                        </div>
                      }
                        <a
                          className="border py-3 px-4 border-slate-200 rounded-lg text-sm font-medium my-2"
                          onClick={() => identidadeBtn.current?.click()}
                        >
                          Escolher arquivo
                        </a>
                      </div>
                      <input
                        type="file"
                        onChange={() => handleChangeFileIdentidade()}
                        ref={identidadeBtn}
                        hidden
                      />
                    </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Dialog>
                <DialogTrigger>
                  <Button
                    className={`bg-green-400 hover:bg-green-500 mb-2 ${step === 3 ? "block" : "hidden"}`}
                  >
                    Cadastrar
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  ...
                  <DialogDescription>
                    <span>
                      Ao clicar <strong>Concluir</strong> você estará aceitando
                      os seguintes termos:
                    </span>
                  </DialogDescription>
                  <div className="border border-slate-300 h-[300px] p-4 rounded-lg overflow-scroll">
                    <span>
                      Autorização para Disponibilização de histórico de crédito
                    </span>
                    <p>
                      Autorizo os gestores de banco de dados de que trata a Lei
                      Nº 12.414, de 9 de junho de 2011, disponibilizar ao
                      Correspondente J M S GROUP LTDA, CNPJ 55.395.267/0001-17 (
                      Zenith Credit ), o meu histórico de crédito, o qual
                      abrangerá os dados financeiros e de pagamentos relativos
                      aos operadores de crédito e obrigações de pagamento
                      adimplidas em seus respectivos vencimentos, e aquelas a
                      vencer, constantes de banco(s) de dados, com a finalidade
                      única e exclusiva de subsidiar a análise e a eventual
                      concessão de crédito,a venda a prazo ou outras transações
                      comerciais e empresariais que impliquem risco financeiro.
                    </p>
                    <p>
                      Esta autorização tem validade de até 3 ( três ) meses a
                      contar da presente data.
                    </p>
                    <p>
                      Estou ciente de que poderei revogar, a qualquer tempo,
                      esta autorização, perante o gestor de banco de dados.
                    </p>
                  </div>
                  <div
                    className={`bg-yellow-200 p-2 rounded-lg border border-yellow-300 ${step === 3 ? "block" : "hidden"}`}
                  >
                    <span className="text-sm text-yellow-600">
                      Atenção: Caso não conclua o registro após apertar
                      Concluir, volte as etapas e verifique se está tudo certo!
                    </span>
                  </div>
                  <Button
